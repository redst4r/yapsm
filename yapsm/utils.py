...
def summary_imbalance(df, treatment_col):
    ...
    avg_group_diffs['std_mean_diff'] = avg_group_diffs['mean_diff']/avg_group_diffs['std']

    # STD only in treatment
    avg_group_diffs['std_treat'] = df.query(f"{treatment_col}==1").std(0)
    avg_group_diffs['std_treat_mean_diff'] = avg_group_diffs['mean_diff']/avg_group_diffs['std_treat']
...
def plot_imbalance(df_original, df_matched, treat):
    ...
    return pn.ggplot(df_merged) + pn.aes(y='index', x='abs_mean_diff', fill='source') + pn.geom_point()
